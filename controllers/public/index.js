...
function readAllMarcas() {
    ...
    fetch(API_CATALOGO + 'readAllTipos', {
        method: 'get'
            ...
            request.json().then(function (response) {
                ...
                if (response.status) {
                    ...
                    response.dataset.map(function (row) {
                        ...
                        url = `articles.html?id=${row.idtipo_producto}&nombre=${row.tipo_producto}`;
                        // Se crean y concatenan las tarjetas con los datos de cada categoría.
                        content += `
                        <div class="col s1 cat"><a href="${url}"><img src="${SERVER}images/carrousel/${row.imagen_tipo}"></a></div>
                        `;
                    });
